...
void setup()
{
  ...
  //abort loop if can't connect for 1minute
  for(int a=0;a<240;a++)
  {
...
void loop()
{
  ...
  int32_t SecondsTicker = (Timestamp.hour() * 60 * 60) + (Timestamp.minute() * 60);
  SecondsElapsed=0;
  yield();
    ...
    if (extractParam(payload, "TIMESTAMP=", '&').length() > 0)
    {
     ...
     if(abs(TSsync.secondstime() - Timestamp.secondstime()) > 10)
     {
      ...
      Serial.printf("our clock is skew from server timestamp! - RESYNCing %d\r\n", TSsync.secondstime() - Timestamp.secondstime());
       ...
       Timestamp = TSsync;
       SecondsTicker = (Timestamp.hour() * 60 * 60) + (Timestamp.minute() * 60);  
     }
    }
    yield();
  ...
  {
    if(is_between(SecondsTicker, settings.RelayLowTime[0], settings.RelayHighTime[0]))
    {
      RelaysCurrentState[0] = false;
#ifdef DEBUG      
  ...
  {
    //
    if(is_between(SecondsTicker, settings.RelayLowTime[1], settings.RelayHighTime[1]))
    {
      RelaysCurrentState[1] = false;
  ...
  {
    //
    if(is_between(SecondsTicker, settings.RelayLowTime[2], settings.RelayHighTime[2]))
    {
      RelaysCurrentState[2] = false;
